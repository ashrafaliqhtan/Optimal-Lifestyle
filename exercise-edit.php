...
// Check if exercise_id is provided and valid
...
try {
    ...
    if ($exercise['user_id'] != $user_id) {
        throw new Exception("You don't have permission to edit this exercise");
    }
} catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($error_message)) {
    try {
        ...
        $stmt = $pdo->prepare("
            UPDATE Exercise 
            SET exercise_type = ?, amount = ?, time = ?
            WHERE exercise_id = ?
        ");
        $stmt->execute([$exercise_type, $amount, $time, $exercise_id]);

        $success_message = "Exercise updated successfully!";
        
        // Refresh exercise data
        $stmt = $pdo->prepare("SELECT * FROM Exercise WHERE exercise_id = ?");
        $stmt->execute([$exercise_id]);
        $exercise = $stmt->fetch(PDO::FETCH_ASSOC);
    } catch (Exception $e) {
        $error_message = $e->getMessage();
...
<head>
    ...
    <title>Edit Exercise - FitnessTracker</title>
...
<body class="d-flex flex-column min-vh-100">
    ...
    <main class="container">
        <div class="row justify-content-center">
            <div class="col-lg-8">
                <div class="card p-4">
                    <h2 class="text-center mb-4">Edit Exercise for Day <?= isset($exercise['day']) ? htmlspecialchars($exercise['day']) : '' ?></h2>
                    ...
                    <?php if ($success_message): ?>
                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            ...
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            <div class="mt-2">
                                <a href="fitness-day.php?id=<?= $exercise['fitness_id'] ?>" class="btn btn-sm btn-success">
                                    Back to Exercises
                                </a>
                            </div>
                        </div>
                    <?php else: ?>
                        <?php if (!empty($exercise)): ?>
                            <form method="POST" action="exercise-edit.php?id=<?= $exercise_id ?>">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="exercise_type" name="exercise_type" required>
                                        ...
                                        <?php foreach ($exercise_types as $type): ?>
                                            <option value="<?= htmlspecialchars($type) ?>" <?= (isset($_POST['exercise_type']) ? $_POST['exercise_type'] : $exercise['exercise_type']) === $type ? 'selected' : '' ?>>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="amount" name="amount" 
                                           min="1" required 
                                           value="<?= isset($_POST['amount']) ? htmlspecialchars($_POST['amount']) : htmlspecialchars($exercise['amount']) ?>">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="time" class="form-control" id="time" name="time" 
                                           required step="1"
                                           value="<?= isset($_POST['time']) ? htmlspecialchars($_POST['time']) : htmlspecialchars($exercise['time']) ?>">
                                    <div class="form-text">Duration of the exercise (HH:MM:SS)</div>
                                </div>
                                
                                <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-4">
                                    <a href="fitness-day.php?id=<?= $exercise['fitness_id'] ?>" class="btn btn-outline-secondary me-md-2">
